		...
		if savedValues_DB.Dps ~= nil and savedValues_DB.Heal ~= nil and savedValues_DB.Macro ~= nil then
			...
			if GetNumGroupMembers() > 0 then
				...
				if name_dps ~= nil then
					...
					-- print("Edited macros [name:|cff00ccff" .. savedValues_DB.Dps .. "|r ] with name: |cff00ccff" .. name_dps .. "|r")
				else
					print("no dps")
				end
				...
				if name_healer ~= nil then
					...
					-- print(
					-- 	"Edited macros [name: |cff00ccff" .. savedValues_DB.Heal .. "|r ] with name: |cff00ccff" .. name_healer .. "|r"
					-- )
				else
					print("no healer")
				end
					print("Edited Heal:" .. savedValues_DB.Dps .. "|r ] with name: |cff00ccff" .. name_dps .. "|r")
...
local function frameEvent()
	...
	MacroNameFrame:SetScript(
		...
		function(self, event, ...)
			...
			elseif (event == "PLAYER_ENTERING_WORLD" and HasLoadedCUFProfiles() and not InCombatLockdown()) or
				(event == "ARENA_PREP_OPPONENT_SPECIALIZATIONS" and HasLoadedCUFProfiles() and not InCombatLockdown()) then
